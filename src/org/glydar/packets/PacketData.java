...
public abstract class PacketData {
    ...
    public <T> List<Byte> getDataList(T dat) {
        ...
        if (dat instanceof Float) {
            ...
            tmpDat.addAll(MiscUtil.toByteList(ByteBuffer.allocate(2).order(ByteOrder.LITTLE_ENDIAN).putShort((Short) dat).array()));
            ...
            byte[] x = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putFloat(vec.getX()).array();
            byte[] y = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putFloat(vec.getY()).array();
            byte[] z = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putFloat(vec.getZ()).array();
    ...
    public <T> T getDataAtIndex(Class<T> dataType, int index) {
        ...
        if (dataType.isAssignableFrom(Float.class)) {
            ...
            return (T) new Short(ByteBuffer.wrap(dt).order(ByteOrder.LITTLE_ENDIAN).getShort());
            ...
            float x = ByteBuffer.wrap(xDat).order(ByteOrder.LITTLE_ENDIAN).getFloat();
            float y = ByteBuffer.wrap(yDat).order(ByteOrder.LITTLE_ENDIAN).getFloat();
            float z = ByteBuffer.wrap(zDat).order(ByteOrder.LITTLE_ENDIAN).getFloat();
